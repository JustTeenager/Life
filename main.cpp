...
float recovery = 1.2;// коэффицент восстановления травы
...
public:
    ...
    void SetFood() {//уменьшает сытость хищников до передвижения
        if (pred.foodp >= 1)
        {
            pred.foodp--;
        }
            ...
            if (herb[i].GetAge() >= 0 && pred[i].GetAge() >= 0) {
                for (int j = 0; j < lenghBase + max; j++) { // проверка на близость хищника и травоядного
                    if (pred[j].GetDmarkp() == true) {
                        if (deathHerb(pred[j].GetX(), pred[j].GetY(), herb[i].GetX(), herb[i].GetY(),
                                      pred[j].GetFoodp(), herb[i].GetDmarkh(), j)) {
                            pred[j].SetFoodp(pred[j].GetFoodp() + 1);
                            herb[i].SetDmarkh();
                        }
                    }
                    if (Born(herb[i].GetX(), herb[i].GetY(), herb[j].GetX(), herb[j].GetY(), herb[i].GetDmarkh(),
                             herb[j].GetDmarkh(), herb[i].CheckRepAge(), herb[j].CheckRepAge()) && (i !=
                                                                                                    j)) {// Процесс рождения:CheckAge проверяет,может ли давать потомство каждый из травоядных,BornHerb-рядом они или нет

                        herb[lenghBase + t] = Herbivores(herb[i].GetX(), herb[i].GetY(), 0, rand() % 7 + 5, 3, 11, 50,
                                                         1);
                        t++;
                        TheEnd++;
                    }
                    if (Born(pred[i].GetX(), pred[i].GetY(), pred[j].GetX(), pred[j].GetY(), pred[i].GetDmarkp(),
                             pred[j].GetDmarkp(), pred[i].CheckRepAge(), pred[j].CheckRepAge()) && (i !=
                                                                                                    j)) // Процесс рождения:CheckAge проверяет,может ли давать потомство каждый из травоядных,BornHerb-рядом они или нет
                    {
